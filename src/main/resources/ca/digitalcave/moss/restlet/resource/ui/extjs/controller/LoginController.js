			...
			form.submit({
				...
				failure: function(form, action) {
					...
					if (response && response.next == "passwordExpired") {
						...
						cmp.up("panel[itemId='authenticate']").down('transientlabel[itemId=messageLogin1]').setDisappearingHtml(Login.translate("INVALID_CREDENTIALS_MESSAGE"));
					}
				}
			});
		}
			...
			form.submit({
				...
				failure: function(form, action) {
					...
					if (response && response.next == "totpBackupCodesNeeded") {
						...
						cmp.up('form').down('transientlabel[itemId=messageTwoFactorToken]').setDisappearingHtml(Login.translate("INVALID_TWO_FACTOR_MESSAGE"));
					}
				}
			});
		}
